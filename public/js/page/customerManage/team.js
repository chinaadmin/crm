...
layui.use(['conf', 'utilFn', 'layer', 'jquery', 'table', 'laydate', 'upload'], function() {
  ...
  var tableConfig = {
    elem: '#dataTable',
    id: 'dataTable',
    url: conf.basePath + conf.selectTeamData,
    method: 'POST',
    where: {
      orderName: '-1',
      orderby: '-1',
    ...
    cols: [
      [
        { type: 'numbers', width: 30, fixed: 'left', align: 'center' },
        { field: 'check', title: '选择', type: 'checkbox', width: 30, fixed: 'left', align: 'center' },
        { field: 'mobilePhone', title: '手机号', width: 100, fixed: 'left', align: 'center', event: 'show', templet: '#tp1' },
        { field: 'realName', title: '姓名', width: 50, align: 'center' },
        { field: 'gender', title: '性别', width: 50, align: 'center', templet: '#tp2' },
        { field: 'age', title: '年龄', width: 50, align: 'center' },
        { field: 'mobilePhoneLocation', title: '归属地', width: 80, align: 'center' },
        { field: 'groupName', title: '客户分组', width: 100, align: 'center' },
        { field: 'lastDate', title: '最近登录', width: 90, align: 'center', templet: '#tp7' },
        { field: 'accountAmount', title: '账户总额', width: 90, align: 'center', sort: true },
        { field: 'usableBalance', title: '账户余额', width: 90, align: 'center', sort: true },
        { field: 'totalGram', title: '黄金克重', width: 90, align: 'center', sort: true },
        { field: 'currentGram', title: '金荷包', width: 90, align: 'center', sort: true },
        { field: 'risefallHold', title: '待收总额', width: 90, align: 'center', sort: true },
        { field: 'followUpResult', title: '跟进结果', width: 100, align: 'center', templet: '#tp3' },
        { field: 'followUpCount', title: '跟进数', width: 80, align: 'center', sort: true },
        { field: 'registerTime', title: '注册时间', width: 90, align: 'center', templet: '#tp4' },
        { field: 'buyFirstTime', title: '首投时间', width: 90, align: 'center', templet: '#tp5' },
        { field: 'firstCallTime', title: '首次通话', width: 90, align: 'center', templet: '#tp10' },
        { field: 'firstRegularBuyTime', title: '首次投定', width: 90, align: 'center', templet: '#tp9' },
        { field: 'lastFollowTime', title: '上次回访', width: 90, align: 'center', templet: '#tp6' },
        { field: 'distributeTime', title: '调配时间', width: 90, align: 'center', templet: '#tp8' },
        { field: 'loginCount', title: '登录次数', width: 90, align: 'center', sort: true },
        { field: 'userName', title: '跟进人', width: 50, align: 'center' },
        { field: 'channelName', title: '渠道', width: 120, align: 'center' }
      ]
    ],
  ...
  var config2 = {
    ...
    cols: [
      [
        ...
        { field: 'groupName', title: '分组名称', width: 120, align: 'center' },
        { field: 'groupDescription', title: '描述', width: 150, align: 'center' },
  ...
  var vm = new Vue({
    ...
    data: {
      ...
      //tab状态
      tabState: 1, //1:未分配,2:已分配,3:回收,100:全部
      //tab内权限
      tabRoles: {},
      //折叠查询
      queryMini: true,
    ...
    methods: {
      ...
      recoveryCustomer: function(type) {
        ...
        if (len == 0) {
          ...
          layer.confirm('选中的客户资料将被回收,确认回收？', { icon: 3, title: '提示', skin: 'crm-model' },
            function(index) {
      ...
      recoveryAll: function() {
        ...
        if (v.count == 0) {
          ...
          layer.confirm(`${v.count}个客户资源将被回收,确认回收?`, { icon: 3, title: '提示', skin: 'crm-model' },
            function(index) {
              var param = {
                recoveryType: '3',
                cstQuery: v.query
      ...
      exportExcel: function() {
        ...
        var fileUrl = `${conf.basePath+conf.exportExcel}?__sid=${token}&${$.param(v.query,true)}&exportType=2`;
      ...
      saveGroupSets: function() {
        ...
        if (msg) {
          layer.msg(msg, { icon: 2, time: 1500, shade: [0.8, '#f0f0f2'] });
          return;
        }
        ...
        $.ajax({
          ...
          if (res.code == 0) {
            ...
            layer.msg('分组设置成功', { icon: 1, time: 1000 });
  ...
  table.on('tool(dataTable)', function(obj) {
    ...
    if (flag) {
      ...
      layer.open({
        ...
        area: ['90%', '90%'],
        scrollbar: false,
        content: `${conf.baseProject}/customerDetails/customerDetail.html?customerId=${userData.customerId}&from=team&to=${vm.roles.unAssignBtnCstDetail.id}`
      });
    }
