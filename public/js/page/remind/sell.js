layui.use(['conf', 'utilFn', 'layer', 'laydate', 'table'], function() {
  ...
  var config = {
    elem: '#dataTable',
    id: 'dataTable',
    url: conf.basePath + conf.selectSellingGoldRemindData,
    method: 'POST',
    where: {
    ...
    cols: [
      [
        ...
        { field: '', title: '呼叫', type: 'space', width: 40, align: 'center', fixed: 'left', templet: '#tp1', event: 'call' },
        { field: 'mobilePhone', title: '客户号码', width: 110, fixed: 'left', align: 'center', event: 'show', templet: '#tp2' },
        { field: 'custName', title: '客户姓名', width: 90, align: 'center' },
        { field: 'sellGram', title: '卖出克重', width: 100, align: 'center', sort: true },
        { field: 'sellPrice', title: '卖出金价', width: 100, align: 'center', sort: true },
        { field: 'sellAmount', title: '卖出金额', width: 100, align: 'center', sort: true },
        { field: 'sellFee', title: '手续费', width: 100, align: 'center', sort: true },
        { field: 'sellSuccessTime', title: '卖出时间', width: 110, align: 'center' },
        { field: 'teamName', title: '组别', width: 100, align: 'center' },
        { field: 'salesName', title: '电销员', width: 90, align: 'center' },
      ...
      if (count > 0) {
        var totalHtml = `<tr>
          <td class="text-center" colspan="4">合计(共${count}条)</td>
          <td class="text-center" colspan="1">${res.map.sellGram}</td>
          <td colspan="1"></td>
  ...
  table.on('tool(dataTable)', function(obj) {
    ...
    if (data.status == '0') {
      ...
      var param = { id: data.id, updatePageType: '6' }
