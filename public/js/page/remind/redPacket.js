layui.use(['conf', 'utilFn', 'layer', 'laydate', 'table', 'util'], function() {
  ...
  var config = {
    elem: '#dataTable',
    id: 'dataTable',
    url: conf.basePath + conf.selectRedPacketRemindData,
    method: 'POST',
    where: {
      startDate: _.getDate('-'),
      endDate: _.getDate('-'),
    ...
    cols: [
      [
        ...
        { field: 'realname', title: '客户姓名', width: 100, align: 'center' },
        { field: 'couponType', title: '类型', width: 100, align: 'center', templet: '#tp3' },
        { field: 'faceValue', title: '面额', width: 150, align: 'center' },
        { field: 'sourceDesc', title: '来源', width: 150, align: 'center' },
        { field: 'status', title: '状态', width: 150, align: 'center', templet: '#tp4' },
        { field: 'createTime', title: '获得日期', width: 150, align: 'center' },
        { field: '', title: '有效时间', width: 280, align: 'center', templet: '#tp5' },
        { field: 'productCategory', title: '可购产品', width: 180, align: 'center', templet: '#tp6' },
        { field: 'validDays', title: '产品期限', width: 150, align: 'center', templet: '#tp7' },
      ]
    ]
  };
  ...
  var vm = new Vue({
    ...
    methods: {
      getAfterDate: function() {
        var d = new Date(this.query.startDate).getTime() + 3 * 24 * 60 * 60 * 1000;
      ...
      exportExcel: function() {
        ...
        var fileUrl = `${conf.basePath+conf.rechargeFailureRemindingExportExcelFile}?__sid=${token}&${$.param(v.query)}`;
  ...
  table.on('tool(dataTable)', function(obj) {
    ...
    if (data.readStatus == '0') {
      ...
      var param = { id: data.couponId, updatePageType: '9' }
