  ...
  var vm = new Vue({
    ...
    methods: {
      ...
      getListData: function() {
        ...
        $.get(conf.basePath + conf.queryWorkorderType).done(function(res) {
      ...
      createType: function(editId, createType) {
        ...
        var urlArr = [conf.addWorkorderType, conf.updateWorkorderTypeName, conf.addWorkorderType];
      ...
      delType: function(id) {
        ...
        layer.confirm('确认删除?', { icon: 3, title: '提示', skin: 'crm-model' }, function(index) {
          ...
          $.post(conf.basePath + conf.delWorkorderType, { id: id }).done(function(res) {
            //登录超时
            if (res.code == -1) {
              window.parent.location.href = '../login.html';
            }
      ...
      exportExcel: function() {
        ...
        var fileUrl = `${conf.basePath+conf.exportWorkorderType}?__sid=${token}`;
