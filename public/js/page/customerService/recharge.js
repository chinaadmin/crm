layui.use(['conf', 'utilFn', 'table'], function() {
  ...
  var baseConfig = {
    elem: '#dataTable',
    id: 'dataTable',
    url: conf.basePath + conf.payWarning,
    method: 'POST',
    where: {
      followStatus: '1',
      mobilePhone: '',
    ...
    cols: [
      [
        ...
        { field: 'isFirstRecharge', title: '有无成功充值过', width: 150, align: 'center', templet: '#tp4' },
        { field: 'recentlyFailDateTime', title: '最近支付失败时间', width: 180, align: 'center' },
        { field: 'recentlyMaxMoney', title: '最近支付失败最大金额', width: 200, align: 'center', sort: true },
        { field: 'recentlyFailTimes', title: '最近支付失败次数', width: 150, align: 'center', sort: true },
      ]
    ]
  };
  ...
  var vm = new Vue({
    ...
    methods: {
      ...
      searchData: function() {
        ...
        config.where.followStatus = this.tabState;
  ...
  table.on('tool(dataTable)', function(obj) {
    ...
    if (flag) {
      ...
      layer.open({
        ...
        area: ['90%', '90%'],
        scrollbar: false,
        content: `${conf.baseProject}/customerDetails/customerDetail.html?customerId=${userData.customerId}&from=recharge&to=${vm.roles.unAssignBtnCstDetail.id}&updateId=${userData.id}`
      });
    }
    if (layEvent == 'call') {
